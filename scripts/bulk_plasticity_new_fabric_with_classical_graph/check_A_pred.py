...
for data_set_id in datset_ids:
    ...
    pred_data = pd.read_csv(root_folder + 'dataset_{}/data_prediction_ave_over_1.csv'.format(data_set_id), delimiter=',')
    eps11 = -exp_data['eps11'].values
    A = np.zeros_like(eps11)
    A_pred = np.zeros_like(eps11)
    ...
    for i in range(num_load_steps):
        ...
        fab_pred = get_sfabric_tensor(pred_data, i)
        fab_true_pred = 7.5 * (np.copy(fab_pred) - np.eye(3) / 3.)
        mag_fab_true_pred = np.sqrt(np.tensordot(fab_true_pred, fab_true_pred))
        N_fab_true_pred = fab_true_pred / mag_fab_true_pred
        
        A[i] = abs(np.tensordot(N_fab_true, N_sig_dev))
        A_pred[i] = abs(np.tensordot(N_fab_true_pred, N_sig_dev))
    ...
    plot_x_y([eps11, eps11], [A, A_pred], x_label='Axial Strain', y_label='A',
                legend_all=['Experiment', 'Predicted Strong Fabric'],
                add_to_save=temp+'strain_Atrue_FabricPred.png'
                )
