...
from src.utilities import get_row_ids, prepare_graphs, plot_with_conf_intrv
...
graph_attr = {'degrAssort': 'Degree Assortativity', 'average_clustering':'Average Clustering',
              'graphDensity':'Graph Density', 'local_efficiency':'Local Efficiency',
              'CN':'Coordination Number', 'transty':'Graph Transitivity'}
dropoutRate = 5
num_mc_trials = 200
...
data_sets_to_predict = [0, 23, 29, 50, 56]
# # init_set_id=0
# # end_set_id=2
...
for data_set_id in data_sets_to_predict:
    ...
    for key, uq_val in out_predic.items():
        ...
        if key in graph_attr.keys():
            plot_with_conf_intrv(out_predic[key], exp_data[key].values, x_field,
                                 num_mc_trials, num_points_in_each_data_set,
                                 x_label='Axial Strain', y_label=graph_attr[key], xlim=None, ylim=None,
                                 add_to_save=temp+'strain_{}_{}dropRate_numMC{}.png'.format(key, dropoutRate, num_mc_trials)
                                )
