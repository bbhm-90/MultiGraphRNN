...
for i, case in enumerate(ann_cases):
    ...
    np.save(temp_root_folder+'train_setwise_ecdf_all.npy', temp)
    temp = np.zeros((len(test_ecdf), 2))
    temp[:,0], temp[:,1] = test_ecdf, y_test
    np.save(temp_root_folder+'test_setwise_ecdf_all.npy', temp)

    add_to_save=temp_root_folder+'setwise_ecdf_all.png'
