...
for i, case in enumerate(ann_cases):
    ...
    # sub graph by subgraph
    for jj, sub_graph_output in enumerate(sub_graph_outputs):
        ff_ids = [features_pred_map[ff] for ff in sub_graph_output]
        train_ecdf = []
        ...
        np.save(temp_root_folder+'train_pointwise_ecdf_{}.npy'.format(case['output_name'][jj]), temp)
        temp = np.zeros((len(test_ecdf), 2))
        temp[:,0], temp[:,1] = test_ecdf, y_test
        np.save(temp_root_folder+'test_pointwise_ecdf_{}.npy'.format(case['output_name'][jj]), temp)
        plot_x_y(x_all=[train_ecdf, test_ecdf],y_all=[y_train, y_test], x_label='mse', y_label='eCDF',
                 legend_all=['train', 'test'], xscale='log',
                 add_to_save=temp_root_folder + 'pointwise_{}.png'.format(case['output_name'][jj]),
                 )
